...
python3 -c 'import os; os.rename("temp/YIE3\x7f\x7f\x7f", "temp/YIE3a"); os.rename("temp/YIE3\x7f\x7f\x7f.inf", "temp/YIE3a.inf")'

distance_pair.py --compress temp/YIE\-AR temp/DATA
ophis -o temp/loader loader.oph
cat temp/loader temp/DATA > temp/LOADER
python3 -c 'open("temp/LOADER.inf", "w").write("$.YIE-AR\t2800\t2800\t%x" % len(open("temp/LOADER").read()))'
