...
python3 -c 't = open("temp/LOADER", "rb").read(); t = t[:0x13f] + b"\x11\xef21:*/BUGLOAD\x0d\xff"; open("temp/LOADER", "wb").write(t)'
sed -i s/14b/14f/g temp/LOADER.inf
python3 -c 't = open("temp/BUGLOAD", "rb").read(); t = t.replace(b"\x4c\x0a\x1e", b"\x4c\xb8\x04") + b"\xa9\x06\x20\xee\xff\x4c\x0a\x1e"; open("temp/BUGLOAD", "wb").write(t)'
