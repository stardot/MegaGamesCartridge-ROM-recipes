...
Copyright (c) 2003-2024, David Boddie <david@boddie.org.uk>
...
class Utilities:
    ...
    def _create_directory(self, path, name = None):
        ...
        try:
            ...
            print('Directory could not be created: %s' % os.sep.join(elements))
            return ""
...
class ADFSnewMap(ADFSmap):
    ...
    def read_catalogue(self, base):
        ...
        if dir_start not in self.dir_markers:
            ...
            if self.verify:
                self.verify_log.append((WARNING, 'Not a directory: %s' % hex(head)))
        ...
        while self.sectors[head+p] != 0:
            ...
            inddiscadd = self._read_new_address(self.sectors[head+p+22:head+p+25])
            newdiratts = self.sectors[head+p+25]

            if inddiscadd == -1:
                if (newdiratts & 0x8) != 0:
                    if self.verify:
                        self.verify_log.append((WARNING, "Couldn't find directory: %s" % name))
                        self.verify_log.append((WARNING, "    at: %x" % (head+p+22)))
                        self.verify_log.append((WARNING, "    file details: %x" % self._str2num(3, self.sectors[head+p+22:head+p+25])))
                        self.verify_log.append((WARNING, "    atts: %x" % newdiratts))

                elif length != 0:
                    if self.verify:
                        self.verify_log.append((WARNING, "Couldn't find file: %s" % name))
                        self.verify_log.append((WARNING, "    at: %x" % (head+p+22)))
                        self.verify_log.append((WARNING, "    file details: %x" % \
                ...
                for start, end in inddiscadd:
                    ...
                    # as a directory.
                    lower_dir_name, lower_files = self.read_catalogue(start)
        ...
        if dir_end not in self.dir_markers:
            if self.verify:
                self.verify_log.append((WARNING,
                    'Discrepancy in directory structure: [%x, %x]' % (head, tail)))

            return b'', files

        dir_name = self._safe(self.sectors[tail+self.sector_size-16:tail+self.sector_size-6])

        parent = self.sectors[tail+self.sector_size-38:tail+self.sector_size-35]

        dir_title = self.sectors[tail+self.sector_size-35:tail+self.sector_size-16]
        ...
        if endseq != dir_seq:
            if self.verify:
                self.verify_log.append((WARNING,
                      'Broken directory: %s at [%x, %x]' % (dir_title, head, tail)))
...
class ADFSoldMap(ADFSmap):
    ...
    def _read_free_space(self):
        ...
        disc_size = self._str2num(3, self.sectors[self.sector_size-4:self.sector_size-1])
        checksum0 = self.sectors[self.sector_size-1]
        ...
        name += self.sectors[base+self.sector_size-10:base+self.sector_size-5]

        disc_id = self._str2num(2, self.sectors[base+self.sector_size-5:base+self.sector_size-3])
        boot = self.sectors[base+self.sector_size-3]
        checksum1 = self.sectors[base+self.sector_size-1]
...
class ADFSdisc(Utilities):
    ...
    def _identify_format(self, adf):
        ...
        if self.verify:
            self.verify_log.append((INFORM, "Checklist for E format discs:"))

            for key, value in checklist.items():
                self.verify_log.append((INFORM, "%s: %s" % (key, ["no", "yes"][value])))

        # If all the tests pass then the disc is an E format disc.
        if sum(checklist.values()) == len(checklist.keys()):
    ...
    def _read_disc_record(self, offset):
        ...
        idlen = self.sectors[offset + 4]
        # Number of bytes per map bit.
        bytes_per_bit = 2 ** self.sectors[offset + 5]
        # LowSector, StartUp, LinkBits, BitSize (size of ID field?)
        bit_size = self.sectors[offset + 6 : offset + 7]
        #print "Bit size: %s" % hex(bit_size)
        # RASkew, BootOpt, Zones
        zones = self.sectors[offset + 9]
        # ZoneSpare, RootDir
        root = self._str2num(3, self.sectors[offset + 13 : offset + 16]) # was 15
        # Identify, SequenceSides, DoubleStep, DiscSize
        ...
        # DiscName
        disc_name = self.sectors[offset + 22 : offset + 32].strip()
        ...
        if inter == 0:
            try:
                for i in range(0, self.ntracks):
                    t += f.read(self.nsectors * self.sector_size)

            except IOError:
                print('Fewer than %i tracks found.' % self.ntracks)
                f.close()
                raise ADFS_exception('Fewer than %i tracks found.' % self.ntracks)
        else:
    ...
    def _read_old_catalogue(self, base):
        ...
        if dir_start not in self.dir_markers:
            if self.verify:
                self.verify_log.append((WARNING, 'Not a directory: %x' % head))
        ...
        while self.sectors[head+p] != 0:
            ...
            if self.disc_type == 'adD':
                inddiscadd = 256 * self._str2num(3, self.sectors[head+p+22:head+p+25])
            else:
                inddiscadd = self.sector_size * self._str2num(
                    3, self.sectors[head+p+22:head+p+25])

            olddirobseq = self.sectors[head+p+25]
        ...
        if self.disc_type == 'adD':
            ...
            parent = self.sector_size*self._str2num(3,
    ...
    def print_catalogue(self, files = None, path = b"$", filetypes = 0):
        ...
        for obj in files:
            ...
            if isinstance(obj, ADFSfile):
                ...
                self.print_catalogue(obj.files, path + b"." + name, filetypes)
        ...
        for obj in objects:
            ...
            if isinstance(obj, ADFSfile):
                ...
                if not filetypes:
                    ...
                    try:
                        inf = open(inf_file, "w")
                        inf.write("$.%s\t%X\t%X\t%X" % (name, obj.load_address,
                            obj.execution_address, obj.length))
                        inf.close()
                    except IOError:
                ...
                self._extract_old_files(obj.files, new_path, filetypes,
    ...
    def _extract_new_files(self, objects, path, filetypes = 0, separator = ",",
        ...
        for obj in objects:
            ...
            if isinstance(obj, ADFSfile):
                ...
                self._extract_new_files(obj.files, new_path, filetypes,
                                        separator, convert_dict)
    ...
    def print_log(self, verbose = 0):
        ...
        if hasattr(self, "disc_map") and 1 in self.disc_map:
            print(self._plural("%i mapped %s found.", [len(self.disc_map[1])],
                  [("defects", "defect", "defects")]))

        # Count the information, warning and error messages in the log.
        informs = sum([(b[0] == INFORM) for a, b in self.verify_log])
        warnings = sum([(b[0] == WARNING) for a, b in self.verify_log])
        errors = sum([(b[0] == ERROR) for a, b in self.verify_log])
