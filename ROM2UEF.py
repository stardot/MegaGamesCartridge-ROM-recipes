...
def read_block(rom, ptr):
    ...
    return bytes(name), number, load_address, exec_address, flags, data, next, ptr
...
if __name__ == "__main__":
    ...
    for rom_file_name in rom_file_names:
        ...
        rom = open(rom_file_name, "rb").read()
        
        try:
            sta_f6 = rom.index(b"\x85\xf6")
            if rom[sta_f6 - 2] == 0xa9:
                ptr_low = rom[sta_f6 - 1]
            else:
                raise IndexError
            
            sta_f7 = rom.find(b"\x85\xf7")
            if rom[sta_f7 - 2] == 0xa9:
                ptr_high = rom[sta_f7 - 1]
            else:
                raise IndexError
            ...
            if result is None:
                ...
                if number == 0:
                    ...
                    if not name.startswith(b"*"):
                        print(repr(name), hex(load_address), hex(exec_address), end=" ")
